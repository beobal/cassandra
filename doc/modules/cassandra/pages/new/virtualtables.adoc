...
|unweighted_ caches |Displays the general unweighted cache information including cache name, capacity, entry_count, hit_count, hit_ratio,
recent_hit_rate_per_second, recent_request_rate_per_second, and request_count.
|===
...
The `unweighted_caches` virtual table lists information about unweighted caches. The five caches presently created are
credentials, jmx_permissions, network_permissions, permissions and roles. A query on the `unweighted_caches` virtual table returns the following details:

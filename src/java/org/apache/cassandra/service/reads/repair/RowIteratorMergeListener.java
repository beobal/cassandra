        ...
        implements UnfilteredRowIterators.MergeListener
{
    ...
    private void applyToPartition(int i, Consumer<PartitionUpdate.Builder> f)
    {
        ...
        {
            if (repairs[i] == null)
                repairs[i] = new PartitionUpdate.MinimalColumnBuilder(command.metadata(), partitionKey, columns, 1);
            f.accept(repairs[i]);
        }
        if (buildFullDiff)
        {
            if (repairs[repairs.length - 1] == null)
                repairs[repairs.length - 1] = new PartitionUpdate.MinimalColumnBuilder(command.metadata(), partitionKey, columns, 1);

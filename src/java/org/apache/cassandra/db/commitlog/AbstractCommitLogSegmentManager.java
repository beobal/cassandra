...
public abstract class AbstractCommitLogSegmentManager
{
    ...
    void start()
    {
        ...
        executor = executorFactory().infiniteLoop("COMMIT-LOG-ALLOCATOR", allocator, true, true);
            ...
            try
            {
                switch (state)
                {
                    ...
                    case NORMAL:
                        ...
                        // synchronized to prevent thread interrupts while performing IO operations
                        synchronized (this)
                        {
                            // if set, clear interrupted status to prevent ClosedByInterruptException in createSegment
                            Thread.interrupted();

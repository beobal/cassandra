...
import org.apache.cassandra.transport.FrameDecoder.Frame;
import org.apache.cassandra.transport.FrameDecoder.FrameProcessor;
import org.apache.cassandra.transport.FrameDecoder.IntactFrame;
import org.apache.cassandra.transport.FrameDecoder.CorruptFrame;
import org.apache.cassandra.transport.InboundMessageCallbacks;
 ...
 */
public class InboundMessageHandler extends ChannelInboundHandlerAdapter implements FrameDecoder.FrameProcessor
{
    private static final Logger logger = LoggerFactory.getLogger(InboundMessageHandler.class);
    private static final NoSpamLogger noSpamLogger = NoSpamLogger.getLogger(logger, 1L, TimeUnit.SECONDS);

    private final org.apache.cassandra.transport.Frame.Decoder internalFrameDecoder;
    ...
    InboundMessageHandler(FrameDecoder decoder,
                          ...
                          org.apache.cassandra.transport.Frame.Decoder internalFrameDecoder,
                          org.apache.cassandra.transport.Message.ProtocolDecoder messageDecoder)
    {
        this.decoder = decoder;
        this.internalFrameDecoder = internalFrameDecoder;
    ...
    {
        // need to replace this with what o.a.c.transport.Message.ProtocolDecoder does
        ByteBuffer buf = bytes.get();
        ByteBuf wrapped = Unpooled.wrappedBuffer(buf);
        try
        {
            org.apache.cassandra.transport.Frame legacyFrame = internalFrameDecoder.decodeFrame(wrapped);
            Message message = messageDecoder.decodeFrame(legacyFrame);
...
//            processLargeMessage(bytes, size, header);
    ...
    private boolean processFirstFrameOfLargeMessage(FrameDecoder.IntactFrame frame, Limit endpointReserve, Limit globalReserve) throws IOException
    {

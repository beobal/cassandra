...
public class DatabaseDescriptor
{
    ...
    private static void applySimpleConfig()
    {
        ...
        if (conf.native_transport_max_protocol_version != Integer.MIN_VALUE)
        {
            try
            {
                ProtocolVersion.decode(conf.native_transport_max_protocol_version, ProtocolVersionLimit.SERVER_DEFAULT);
                logger.info("Native transport max negotiable version statically limited to {}", conf.native_transport_max_protocol_version);
            }
            catch (Exception e)
            {
